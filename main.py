...
async def count(ctx,user):
    try:
        ...
        for row in dataRows:
            if(row[1]=='Casual'):
                casualStr += bold("[")+"時間:"+bold(str(row[6])[:10])+"|勝/負:"+bold(str(row[2])+"/"+str(row[3]))+"|殺/死:"+bold(str(row[4])+"/"+str(row[5]))+bold("]")+newLine()
            else:
                rankStr += bold("[")+"時間:"+bold(str(row[6])[:10])+"|勝/負:"+bold(str(row[2])+"/"+str(row[3]))+"|殺/死:"+bold(str(row[4])+"/"+str(row[5]))+bold("]")+newLine()
